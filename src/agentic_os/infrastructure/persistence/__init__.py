...
def get_eventlog_repo() -> EventLogRepository:
    ...
    Opciones: "memory" | "jsonl" (default) | "postgres" | "supabase".
    Postgres/Supabase degradan a jsonl (fail-safe) si el driver/BD no
    está disponible, de modo que cambiar EVENTLOG_IMPL no rompe nunca el kernel.
    """
    ...
    if impl == "supabase":
        from .supabase import SupabaseEventLog

        repo = SupabaseEventLog()
        return repo if repo.available else JsonlEventLog()
    if impl == "jsonl":
