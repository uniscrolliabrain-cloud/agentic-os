# 004/100 - FASE 1.4 — UNIFICACIÓN TEMPORAL
FASE 1 - SANITIZACIÓN Y CIMENTACIÓN DEL KERNEL
Estado: - [ ] PENDIENTE

PROTOCOLO CLINE:
1. Inspecciona repo real antes de tocar codigo
2. No copies codigo de spec ciegamente
3. NUNCA pass/TODO/.../Any/type:ignore/except:pass/bypass
4. UserContext->Policy->Command->Executor->ConnectorRouter->Connector
5. LLM!=Executor
6. Todo cambio con tests + STATUS/ARCHIVOS/CAMBIOS/TESTS/RESULTADO/INVARIANTES/RIESGOS


## PROMPT PARA CLINE
```text
# TAREA 004/100 - FASE 1.4 — UNIFICACIÓN TEMPORAL

## PROTOCOLO OBLIGATORIO ANTES DE CODIFICAR
1. Lee el estado REAL del repo: inspecciona src/, tests/, imports actuales.
2. No copies código de la especificación ciegamente. Si ya existe implementación superior, consérvala y adapta.
3. Localiza tests relacionados antes de tocar código.
4. No introduzcas pass, TODO, ..., Any, type: ignore, except: pass ni bypasses.

## CONTEXTO ARQUITECTÓNICO
Fase 1: SANITIZACIÓN Y CIMENTACIÓN DEL KERNEL
Invariante clave: LLM != Executor, Agent != Connector, Text != Command, Command != Permission. Todo efecto externo pasa por UserContext -> Policy -> Command -> Executor -> ConnectorRouter -> Connector.

## TU TAREA ESPECÍFICA
FASE 1.4 — UNIFICACIÓN TEMPORAL

Haz una búsqueda global de:

datetime.utcnow()
datetime.now(
timezone.utc
datetime.now(timezone.utc)

El sistema debe utilizar como abstracción canónica:

src/agentic_os/kernel/types/time.py

y concretamente now_utc() cuando corresponda.

Prioriza:

- rest.py
- scheduler.py
- connectors/webhook/
- token_manager.py
- modelos de dominio
- eventos
- persistencia

No cambies semántica temporal innecesariamente.

La finalidad es que el tiempo del sistema pueda ser controlado/integrado mediante la abstracción del Kernel.

Si algún módulo necesita una representación temporal distinta, documenta la razón antes de modificarlo.

Añade tests que demuestren que now_utc() es la fuente canónica en los caminos modificados.

Ejecuta pytest.

## ENTREGABLES OBLIGATORIOS AL FINALIZAR
Debes responder con:
STATUS: PASS/FAIL
ARCHIVOS MODIFICADOS: lista
CAMBIOS: bullet points
TESTS: qué tests ejecutaste y resultado
RESULTADO: resumen técnico
INVARIANTES PROTEGIDAS: cuáles
RIESGOS: si quedan

## COMANDOS A EJECUTAR
pytest tests/ -k <relevante> -q
ruff check src tests
mypy src --ignore-missing-imports (si aplica)

No avances al siguiente prompt hasta validar este.

```

## SPEC ORIGINAL
```text
FASE 1.4 — UNIFICACIÓN TEMPORAL

Haz una búsqueda global de:

datetime.utcnow()
datetime.now(
timezone.utc
datetime.now(timezone.utc)

El sistema debe utilizar como abstracción canónica:

src/agentic_os/kernel/types/time.py

y concretamente now_utc() cuando corresponda.

Prioriza:

- rest.py
- scheduler.py
- connectors/webhook/
- token_manager.py
- modelos de dominio
- eventos
- persistencia

No cambies semántica temporal innecesariamente.

La finalidad es que el tiempo del sistema pueda ser controlado/integrado mediante la abstracción del Kernel.

Si algún módulo necesita una representación temporal distinta, documenta la razón antes de modificarlo.

Añade tests que demuestren que now_utc() es la fuente canónica en los caminos modificados.

Ejecuta pytest.
```

## CHECKLIST
- [ ] Inspeccione repo
- [ ] No copie ciego
- [ ] Tests verdes
- [ ] Entregue STATUS etc
